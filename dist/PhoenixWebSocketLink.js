...
var linkUtils = createCommonjsModule(function (module, exports) {
...
function validateLink(link$$1) {
    if (link$$1 instanceof link.ApolloLink && typeof link$$1.request === 'function') {
        return link$$1;
    }
    else {
        throw new LinkError('Link does not extend ApolloLink and implement request', link$$1);
    }
}
...
var LinkError = (function (_super) {
    ...
    function LinkError(message, link$$1) {
...
function toLink(link$$1) {
    if (typeof link$$1 === 'function') {
        return new link.FunctionLink(link$$1);
    }
    else {
        return link$$1;
    }
...
function isTerminating(link$$1) {
    return link$$1.request.length <= 1;
}
...
unwrapExports(linkUtils);
var linkUtils_1 = linkUtils.validateLink;
var linkUtils_2 = linkUtils.validateOperation;
var linkUtils_3 = linkUtils.LinkError;
var linkUtils_4 = linkUtils.toLink;
var linkUtils_5 = linkUtils.isTerminating;
var linkUtils_6 = linkUtils.makePromise;
...
var location$1 = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", {
  value: true
});
...
unwrapExports(location$1);
var location_1 = location$1.getLocation;
...
message, nodes, source, positions, path, originalError) {
  ...
  if (_source2 && _positions) {
    _locations = _positions.map(function (pos) {
      return (0, location$1.getLocation)(_source2, pos);
    });
  }
...
function syntaxError(source, position, description) {
  var location = (0, location$1.getLocation)(source, position);
...
unwrapExports(zenObservable);
var zenObservable_1 = zenObservable.Subscription;
var zenObservable_2 = zenObservable.SubscriptionObserver;
...
var ApolloLink = (function () {
    ...
    ApolloLink.from = function (links) {
        ...
        return links.map(linkUtils.toLink).reduce(function (x, y) { return x.concat(y); });
    };
    ApolloLink.empty = function () {
        return new FunctionLink(function (op, forward) { return zenObservable.default.of(); });
    };
    ApolloLink.passthrough = function () {
        return new FunctionLink(function (op, forward) { return (forward ? forward(op) : zenObservable.default.of()); });
    };
    ApolloLink.split = function (test, left, right) {
        if (right === void 0) { right = ApolloLink.passthrough(); }
        var leftLink = linkUtils.validateLink(linkUtils.toLink(left));
        var rightLink = linkUtils.validateLink(linkUtils.toLink(right));
        if (linkUtils.isTerminating(leftLink) && linkUtils.isTerminating(rightLink)) {
            return new FunctionLink(function (operation) {
                return test(operation)
                    ? leftLink.request(operation) || zenObservable.default.of()
                    : rightLink.request(operation) || zenObservable.default.of();
            });
        }
        else {
            return new FunctionLink(function (operation, forward) {
                return test(operation)
                    ? leftLink.request(operation, forward) || zenObservable.default.of()
                    : rightLink.request(operation, forward) || zenObservable.default.of();
            });
        }
    };
    ...
    ApolloLink.prototype.concat = function (next) {
        var _this = this;
        linkUtils.validateLink(this);
        if (linkUtils.isTerminating(this)) {
            console.warn(new linkUtils.LinkError("You are calling concat on a terminating link, which will have no effect", this));
            return this;
        }
        var nextLink = linkUtils.validateLink(linkUtils.toLink(next));
        if (linkUtils.isTerminating(nextLink)) {
            return new FunctionLink(function (operation) {
                return _this.request(operation, function (op) { return nextLink.request(op) || zenObservable.default.of(); }) || zenObservable.default.of();
            });
        }
        else {
            return new FunctionLink(function (operation, forward) {
                return (_this.request(operation, function (op) {
                    return nextLink.request(op, forward) || zenObservable.default.of();
                }) || zenObservable.default.of());
            });
        }
    };
    return ApolloLink;
...
function execute(link, operation) {
    ...
    linkUtils.validateOperation(copy);
    if (!copy.context) {
        copy.context = {};
    }
    if (!copy.variables) {
    ...
    return link.request(transformOperation(copy)) || zenObservable.default.of();
}
...
function transformOperation(operation) {
    ...
    if (typeof operation.query === 'string') {
        transformedOperation = __assign({}, operation, { query: src.default(operation.query) });
    }
    else {
...
unwrapExports(link);
var link_1 = link.ApolloLink;
var link_2 = link.execute;
var link_3 = link.FunctionLink;
...
exports.execute = link.execute;
exports.ApolloLink = link.ApolloLink;

exports.makePromise = linkUtils.makePromise;

exports.Observable = zenObservable.default;
__export(zenObservable);
exports.default = link.ApolloLink;

});

unwrapExports(lib);
var lib_1 = lib.execute;
var lib_2 = lib.ApolloLink;
var lib_3 = lib.makePromise;
var lib_4 = lib.Observable;
...
var PhoenixWebSocketLink = function () {
  /**
   * @param {Object} opts 
  ...
  function PhoenixWebSocketLink(opts) {
    ...
    this._socket = opts.socket || new phoenix_1(opts.uri, opts);
  ...
  _createClass(PhoenixWebSocketLink, [{
    ...
    value: function request(_ref) {
      ...
      query = printer_1(query);
      return new lib_4(function (observer) {
        _newArrowCheck(this, _this);

        this._channel.push('doc', { operationName: operationName, query: query, variables: variables }).receive('ok', function (response) {
          _newArrowCheck(this, _this);

          // console.log('operation response', response)
          observer.next(response);
          observer.complete();
        }.bind(this)).receive('ignore', function (response) {
          _newArrowCheck(this, _this);

          observer.next(response);
          observer.complete();
        }.bind(this)).receive('error', observer.error.bind(observer)).receive('timeout', observer.error.bind(observer));
      }.bind(this));
    }
...
export default PhoenixWebSocketLink;
